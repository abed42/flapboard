...
import { playFlapClick, preloadFlapSound } from "@/lib/flap-sound";
...
const FlapCell = React.memo(function FlapCell({
  ...
  // Read through a ref so toggling sound never restarts an in-flight flip.
  ...
  useEffect(() => {
    ...
    const runStep = (i: number) => {
      ...
      if (soundRef.current) playFlapClick();
  ...
  return (
    <div className="flex aspect-[3/6] flex-col overflow-hidden rounded-[2px] border border-neutral-300 md:rounded-[3px] md:border-2 dark:border-black">
      {/* Flap content area */}
      <div className="relative flex-1 [perspective:100px] [transform-style:preserve-3d]">
      ...
      <div className="h-2 w-full bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.15rem)] [mask-image:linear-gradient(to_top,black_50%,transparent_100%)] text-neutral-400 opacity-20 md:h-4 md:bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.2rem)] dark:text-black dark:opacity-100" />
    </div>
  );
},
(prevProps, nextProps) =>
...
export interface TextFlippingBoardProps {
  ...
  duration?: number;
  /** Play the mechanical click sprite on each flip. */
  sound?: boolean;
  /** Number of flap rows. Defaults to 6. */
  rowCount?: number;
  /** Number of flap columns. Defaults to 22. */
  colCount?: number;
...
export function TextFlippingBoard({
  ...
  const board = useMemo(() => {
    ...
    if (text) {
      const lines = wrapText(text, colCount).slice(0, rowCount);
      ...
      lines.forEach((line, i) => {
        ...
        const startCol = Math.max(
          0,
          Math.floor((colCount - parsed.length) / 2),
        );
        parsed.forEach((cell, c) => {
          if (startCol + c < colCount) {
